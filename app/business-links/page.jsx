...
function EditUserLinks() {
  ...
  const AddLink = async (e) => {
    ...
    try {
      ...
      toast.error(t('businessLinks.failedToAddLink'));
    } finally {
      setLoadingAction(null);
    }
  };
  ...
  const UpdateLink = async (e) => {
    ...
    try {
      ...
      toast.error(t('businessLinks.failedToUpdateLink'));
    } finally {
      setLoadingAction(null);
    }
  };
        ...
        <section className='p-2 md:p-4 rounded-lg bg-gray-100 w-full max-w-[98%] md:w-[110vh] user-select-none mx-auto md:mx-0 text-gray-800 shadow-xl'>
          ...
          <div className={`mx-0 md:mx-3 p-3 md:p-6 bg-white rounded-xl shadow-sm border border-gray-200 mb-6 md:mb-8`}>
            <form onSubmit={AddLink} className="space-y-3 md:space-y-5">
              <div>
                <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1">{t('businessLinks.linkTitle') || "Title"}</label>
                <input
                  type="text"
                  value={namelink}
                  onChange={(e) => setNamelink(e.target.value)}
                  required
                  placeholder={t('businessLinks.exampleWebsite')}
                  maxLength={100}
              ...
              <div>
                <label className="block text-xs md:text-sm font-medium text-gray-700 mb-1">{t('businessLinks.link') || "Link"}</label>
                <input
                  type="url"
                  value={link}
                  placeholder={t('businessLinks.exampleUrl')}
                  maxLength={100}
            ...
            {(loadingAll) ? (
              ...
              userLinks.length > 0 ? (
                  ...
                  >
                    ...
                    <div className='flex items-center gap-1.5 md:gap-2 shrink-0 ml-1.5 md:ml-2'>
                      <button
                        ...
                        title={t('businessLinks.edit') || "Edit"}
                      >
                        <Edit3 className="w-3 h-3 md:w-3.5 md:h-3.5" />
                        <span className="hidden sm:inline">{t('businessLinks.edit') || "Edit"}</span>
                      </button>
                      <button
                        onClick={() => {
                          setDeleteId(lnk._id);
                        ...
                        title={t('businessLinks.delete') || "Delete"}
                      >
                        {loadingAction === `delete-${lnk._id}` ? (
                          ...
                          <>
                            ...
                            <span className="hidden sm:inline">{t('businessLinks.delete') || "Delete"}</span>
                          </>
                        )}
                      </button>
                    </div>
                ...
                <div className="text-center py-8 text-gray-500 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                  <p>{t('businessLinks.noLinksFound') || "No links added yet."}</p>
                </div>
              )
            }
          </div>
      ...
      {isEditModalOpen && createPortal(
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 relative" dir={userDetails?.displayLanguage === 'ar' ? 'rtl' : 'ltr'}>
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-6">{t('businessLinks.updateLink') || "Update Link"}</h2>

            <form onSubmit={UpdateLink} className="space-y-5">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('businessLinks.linkTitle') || "Title"}</label>
                <input
                  type="text"
                  value={editNamelink}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('businessLinks.link') || "Link"}</label>
                <input
                  type="url"
                  value={editLinkUrl}
                  placeholder={t('businessLinks.exampleUrl')}
              ...
              >
                {loadingAction === 'update' ? (
                  ...
                  t('businessLinks.updateLink') || "Update"
                )}
              </button>
            </form>
          </div>
