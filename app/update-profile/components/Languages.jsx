...
export default function Languages({ userData, setUserDetails }) {
  ...
  const saveLanguages = async () => {
    ...
    const validLanguages = languages.filter(l => l.value.trim() !== "").map(l => l.value);
    ...
    try {
      ...
      // Sync local state
      setLanguages(validLanguages.map((l, i) => ({
        value: l,
        localId: languages[i] && languages[i].value === l ? languages[i].localId : `id-${Math.random().toString(36).substr(2, 9)}`
      })));
  ...
  return (
    <div className="space-y-4" dir={userData?.displayLanguage === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-gradient-to-br from-cyan-50 to-blue-50 p-2 md:p-4 rounded-xl border border-cyan-200 space-y-3">
              ...
              {languages.map((langObj, index) => (
                <SortableItem key={langObj.localId} id={langObj.localId}>
                  {({ attributes, listeners, setActivatorNodeRef, isDragging }) => (
                    <div className={`flex gap-0.5 sm:gap-2 items-center transition-all duration-500 rounded-lg p-0.5 sm:p-1 ${isDragging ? 'bg-cyan-50 scale-[1.02] shadow-lg ring-1 ring-cyan-300 z-50' : (movedItemIndex === index ? 'bg-cyan-100 ring-2 ring-cyan-300' : 'hover:bg-cyan-50/50')}`}>
                      ...
                      <div
                        ...
                        {...listeners}
                        {...attributes}
                        className="flex-shrink-0 w-4 h-4 sm:w-8 sm:h-8 flex items-center justify-center bg-cyan-100 text-cyan-700 rounded-full font-bold text-[8px] sm:text-sm border border-cyan-200 shadow-sm cursor-grab active:cursor-grabbing hover:bg-cyan-200 transition-colors touch-none"
                      >
                        {index + 1}
                      </div>
        ...
        >
          <Plus className="w-4 h-4 sm:w-[18px] sm:h-[18px]" /> {languages.length >= 10 ? "10 Max" : t('languages.addLanguage')}
        </button>
      </div>
            ...
            <div className="w-full">
              <div className="text-gray-600 mb-6 text-base leading-relaxed">
                <div className="flex flex-col gap-2">
                  ...
                  <span className="font-bold text-black border-t pt-2 mt-1 break-all">"{languages[itemToDelete]?.value || t('languages.thisLanguage') || "this language"}"</span>
                </div>
              </div>
            </div>
