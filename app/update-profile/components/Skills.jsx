...
export default function Skills({ userData, setUserDetails }) {
  ...
  const UpSkills = async () => {
    // Filter empty skills
    const validSkills = skills.filter(s => s.value.trim() !== "").map(s => s.value);
    ...
    try {
      ...
      setSkills(validSkills.map((s, i) => ({
        value: s,
        localId: skills[i] && skills[i].value === s ? skills[i].localId : `id-${Math.random().toString(36).substr(2, 9)}`
      })));
              ...
              {skills.map((skillObj, index) => (
                <SortableItem key={skillObj.localId} id={skillObj.localId}>
                  {({ attributes, listeners, setActivatorNodeRef, isDragging }) => (
                    <div className={`flex gap-0.5 sm:gap-2 items-center transition-all duration-500 rounded-lg p-0.5 sm:p-1 ${isDragging ? 'bg-purple-50 scale-[1.02] shadow-lg ring-1 ring-purple-300 z-50' : (movedItemIndex === index ? 'bg-purple-100 ring-2 ring-purple-300' : 'hover:bg-purple-50/50')}`}>
                      {/* Draggable Number Handle */}
                      <div
                        ref={setActivatorNodeRef}
                        {...listeners}
                        {...attributes}
                        className="flex-shrink-0 w-4 h-4 sm:w-8 sm:h-8 flex items-center justify-center bg-purple-100 text-purple-700 rounded-full font-bold text-[8px] sm:text-sm border border-purple-200 shadow-sm cursor-grab active:cursor-grabbing hover:bg-purple-200 transition-colors touch-none"
                      >
                        {index + 1}
                      </div>
                      <input
            ...
            <div className="w-full">
              <div className="text-gray-600 mb-6 text-base leading-relaxed">
                <div className="flex flex-col gap-2">
                  ...
                  <span className="font-bold text-black border-t pt-2 mt-1 break-all">"{skills[itemToDelete]?.value || "this skill"}"</span>
                </div>
              </div>
            </div>
