...
export default function Experience({ userData, setUserDetails }) {
  ...
  const saveExperienceItem = async (index) => {
    ...
    try {
      ...
      const res = await axios.put('/api/proxy/users/update/experience/item', itemData);

      const newExperienceList = res.data.experience;
      const mergedExperience = newExperienceList.map((e, i) => {
        const oldE = experience.find(op => op._id === e._id) || (experience[i] && !experience[i]._id ? experience[i] : null);
        return {
          ...e,
          collapsed: oldE ? oldE.collapsed : true,
          localId: oldE ? oldE.localId : `id-${Math.random().toString(36).substr(2, 9)}`
        };
      });
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await axios.delete(`/api/proxy/users/update/experience/${item._id}`);

      const newExperienceList = res.data.experience;
      // Re-map localIds
      setExperience(newExperienceList.map(e => ({
        ...e,
        collapsed: true,
        localId: e._id || `id-${Math.random().toString(36).substr(2, 9)}`
      })));
  ...
  const saveOrder = async () => {
    ...
    try {
      ...
      await axios.put(`/api/proxy/users/update/experience/order`, { experience: experienceWithIds });
  ...
  return (
    <div className="space-y-4" dir={userData?.displayLanguage === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-2 md:p-4 rounded-xl border border-amber-200 space-y-3">
                    ...
                    <div
                      ...
                      className={`bg-white pb-1 border rounded-xl shadow-sm hover:shadow-md transition-all duration-500 overflow-hidden ${deletingIds.has(exp._id) ? 'opacity-60 grayscale pointer-events-none ring-2 ring-red-100 border-red-200 scale-[0.99]' :
                          validationErrors[index] ? 'border-red-300 ring-1 ring-red-200' :
                            (isDragging ? 'border-amber-400 bg-amber-50 shadow-lg scale-[1.02] z-50' :
                      ...
                      <div className="p-1 sm:p-4 flex items-center justify-between gap-1 sm:gap-3 bg-white">
                        <div className="flex-grow flex items-center gap-1 sm:gap-3">
                          ...
                          <div
                            ...
                            {...listeners}
                            {...attributes}
                            className="flex-shrink-0 w-4 h-4 sm:w-8 sm:h-8 flex items-center justify-center bg-amber-100 text-amber-700 rounded-full font-bold text-[8px] sm:text-sm border border-amber-200 shadow-sm cursor-grab active:cursor-grabbing hover:bg-amber-200 transition-colors touch-none"
                        ...
                        <div className="flex items-center gap-0 sm:gap-2 flex-shrink-0">
                          ...
                          >
                            {exp.collapsed ? <Pencil className="w-2.5 h-2.5 sm:w-[16px] sm:h-[16px]" /> : <X className="w-2.5 h-2.5 sm:w-[16px] sm:h-[16px]" />}
                          ...
                          >
                            {deletingIds.has(exp._id) ? <Loader size={12} className="animate-spin w-2.5 h-2.5 sm:w-[16px] sm:h-[16px]" /> : <Trash2 className="w-2.5 h-2.5 sm:w-[16px] sm:h-[16px]" />}
                          </button>
                        </div>
                      </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5 sm:gap-2">
                            <div>
                              ...
                              {validationErrors[index]?.startDate && (
                                <p className="text-red-500 text-[9px] sm:text-xs mt-1 sm:mt-1.5 flex items-center gap-1 sm:gap-1.5 font-medium animate-in slide-in-from-top-1">
                                  <AlertCircle className="w-3 h-3 sm:w-[14px] sm:h-[14px]" />
                                  {t('experience.startDateRequired') || "Start date is required"}
                                </p>
                              )}
                            </div>
                            ...
                            <div>
                              ...
                              {validationErrors[index]?.endDate && (
                                <p className="text-red-500 text-[9px] sm:text-xs mt-1 sm:mt-1.5 flex items-center gap-1 sm:gap-1.5 font-medium animate-in slide-in-from-top-1">
                                  <AlertCircle className="w-3 h-3 sm:w-[14px] sm:h-[14px]" />
                                  {t('experience.endDateRequired') || "End date is required"}
                                </p>
                              )}
                            </div>
                          </div>
        ...
        >
          <Plus className="w-4 h-4 sm:w-[18px] sm:h-[18px]" /> {experience.length >= 10 ? "10 Max" : t('experience.addExperience')}
        </button>
      </div>
      ...
      {experience.filter(e => e._id).length >= 2 && (
        <div className="flex justify-end py-4 border-b-2 border-gray-200">
          ...
          >
            {loading ? (
              <>
                <Loader size={20} className="animate-spin w-4 h-4 sm:w-5 sm:h-5" /> {t('experience.saving')}
              </>
            ) : (
