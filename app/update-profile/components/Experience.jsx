...
export default function Experience({ userData, setUserDetails }) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const newExperienceList = res.data.experience;
      setExperience(newExperienceList.map(e => ({ ...e, collapsed: true })));
  ...
  const saveOrder = async () => {
    ...
    try {
      ...
      if (setUserDetails) {
        setUserDetails(prev => ({
          ...prev,
          experience: experience.map(({ collapsed, ...rest }) => rest)
        }));
      }
  ...
  return (
    <div className="space-y-4" dir={userData?.displayLanguage === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-2 md:p-4 rounded-xl border border-amber-200 space-y-3">
            ...
            >
              ...
              >
                <div className="flex-grow flex items-center gap-1 sm:gap-3">
                  <div className="flex-shrink-0 w-4 h-4 sm:w-8 sm:h-8 flex items-center justify-center bg-amber-100 text-amber-700 rounded-full font-bold text-[8px] sm:text-sm border border-amber-200 shadow-sm">
                    {index + 1}
                  </div>
                ...
                <div className="p-3 md:p-4 border-t border-gray-100 space-y-3 bg-gray-50/50 animate-in fade-in slide-in-from-top-1 duration-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5 sm:gap-2">
                    <input
                      ...
                      className="px-2 py-1.5 sm:px-3 sm:py-2 text-[10px] sm:text-base border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white transition"
                    />
                    <input
                      type="text"
        ...
        {/* Add Experience Button Moved to Bottom */}
        <button
          type="button"
