...
export default function Education({ userData, setUserDetails }) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setEducation(newEducationList.map(e => ({
        ...e,
        collapsed: true,
        localId: e._id || `id-${Math.random().toString(36).substr(2, 9)}`
      })));
  ...
  return (
    <div className="space-y-4" dir={userData?.displayLanguage === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-gradient-to-br from-blue-50 to-purple-50 p-2 md:p-4 rounded-xl border border-blue-200 space-y-3">
                    ...
                    <div
                      ...
                      className={`bg-white pb-1 border rounded-xl shadow-sm hover:shadow-md transition-all duration-500 overflow-hidden ${deletingIds.has(edu._id) ? 'opacity-60 grayscale pointer-events-none ring-2 ring-red-100 border-red-200 scale-[0.99]' :
                          validationErrors[index] ? 'border-red-300 ring-1 ring-red-200' :
                            (isDragging ? 'border-blue-400 bg-blue-50 shadow-lg scale-[1.02] z-50' :
                      ...
                      <div className="p-1 sm:p-4 flex items-center justify-between gap-1 sm:gap-3 bg-white">
                        <div className="flex-grow flex items-center gap-1 sm:gap-3">
                          ...
                          <div
                            ...
                            {...listeners}
                            {...attributes}
                            className="flex-shrink-0 w-4 h-4 sm:w-8 sm:h-8 flex items-center justify-center bg-blue-100 text-blue-700 rounded-full font-bold text-[8px] sm:text-sm border border-blue-200 shadow-sm cursor-grab active:cursor-grabbing hover:bg-blue-200 transition-colors touch-none"
                        ...
                        <div className="flex items-center gap-0 sm:gap-2 flex-shrink-0">
                          ...
                          >
                            {edu.collapsed ? <Pencil className="w-2.5 h-2.5 sm:w-[16px] sm:h-[16px]" /> : <X className="w-2.5 h-2.5 sm:w-[16px] sm:h-[16px]" />}
                          ...
                          >
                            {deletingIds.has(edu._id) ? <Loader size={12} className="w-2.5 h-2.5 sm:w-[16px] sm:h-[16px] animate-spin" /> : <Trash2 className="w-2.5 h-2.5 sm:w-[16px] sm:h-[16px]" />}
                          </button>
                        </div>
                      </div>
        ...
        >
          <Plus className="w-4 h-4 sm:w-[18px] sm:h-[18px]" /> {education.length >= 10 ? "10 Max" : t('education.addEducation')}
        </button>
      </div>
      ...
      {education.filter(e => e._id).length >= 2 && (
        <div className="flex justify-end py-4 border-b-2 border-gray-200">
          ...
          >
            {loading ? (
              <>
                <Loader size={20} className="animate-spin w-4 h-4 sm:w-5 sm:h-5" /> {t('education.saving')}
              </>
            ) : (
