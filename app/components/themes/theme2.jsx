...
export default function Theme2({ userDetails, userLinks }) {
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-2 p-1 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 w-full md:w-auto">
              {(() => {
                ...
                return tabsToRender.map((tab) => {
                  const Icon = tab.icon || User; // Fallback icon
                  return (
                    <button
                      key={tab.key}
                      ...
                      {(userDetails.displayEmail || userDetails.email) && (
                        <div className="flex items-center gap-3 md:gap-4 p-3 md:p-4 bg-white/5 rounded-2xl overflow-hidden">
                          ...
                          <div className="min-w-0">
                            ...
                            <a href={`mailto:${userDetails.displayEmail || userDetails.email}`} className="text-white hover:text-blue-300 transition break-all text-sm md:text-base">
                              {userDetails.displayEmail || userDetails.email}
                            </a>
                          </div>
                        </div>
              ...
              {activeTab === "certificates" && (
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {userDetails.certificates?.map((cert, i) => (
                    <div
                      key={i}
                      className="relative bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl overflow-hidden hover:border-blue-500/30 transition-all duration-300 group flex flex-col"
                    >
                      {cert.cfimage && (
