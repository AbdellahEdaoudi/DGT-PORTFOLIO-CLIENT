...
export default function Theme3({ userDetails, userLinks }) {
  ...
  return (
    <div
      ...
      <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
        ...
        <div
          className="absolute top-0 right-0 w-[500px] h-[500px] bg-purple-600/20 rounded-full mix-blend-screen filter blur-[100px] opacity-30 animate-blob"
        ...
        <header className="pt-8 pb-10">
          ...
          <div className="flex flex-col-reverse md:flex-row gap-10 items-center justify-between mb-8">
            <div className={`flex-1 text-center  ${userDetails?.displayLanguage === 'ar' ? 'md:text-right' : 'md:text-left'}`}>
              ...
              <h1 className="text-5xl md:text-7xl font-bold mb-6 tracking-tight leading-tight bg-clip-text text-transparent bg-gradient-to-r from-white via-pink-100 to-purple-200">
                {userDetails?.fullname}
              </h1>
              <p className="text-2xl md:text-3xl font-bold text-pink-300/90 mb-6 flex items-center justify-center md:justify-start gap-2">
            ...
            <div className="relative group">
              <div className="absolute -inset-1 bg-gradient-to-r from-pink-600 to-purple-600 rounded-[2rem] blur opacity-40 group-hover:opacity-75 transition duration-1000 group-hover:duration-200 animate-tilt"></div>
                      ...
                      {(userDetails.displayEmail || userDetails.email) && (
                        <div className="flex items-center gap-3 md:gap-4 p-3 md:p-4 bg-white/5 rounded-2xl overflow-hidden">
                          ...
                          <div className="min-w-0">
                            ...
                            <a href={`mailto:${userDetails.displayEmail || userDetails.email}`} className="text-white hover:text-pink-300 transition break-all text-sm md:text-base">
                              {userDetails.displayEmail || userDetails.email}
                            </a>
                          </div>
                        </div>
              ...
              {activeTab === "services" && (
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {userDetails.services?.map((service, i) => (
                    <div
                      key={i}
                      className="group p-6 bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl hover:bg-white/10 hover:border-pink-500/50 transition-all duration-300 hover:-translate-y-1"
                    >
                      <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-pink-500/20 to-purple-500/20 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
              ...
              {activeTab === "projects" && (
                <div className="space-y-8">
                  {userDetails.projects?.map((project, i) => (
                    <div
                      key={i}
                      className="group bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl overflow-hidden hover:border-pink-500/30 transition-all duration-500"
                    >
                      <div className="flex flex-col md:flex-row">
                        ...
                        <div className="flex-1 p-4 md:p-8">
                          <div className="flex justify-between items-start mb-4">
                            <h3 className="text-2xl text-white group-hover:text-pink-300 transition-colors">
                              {project.title}
                            </h3>
                            {project.link && (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="p-2 bg-white/10 rounded-full hover:bg-pink-500 hover:text-white transition-all duration-300"
                              >
                                <Globe size={20} />
                              </a>
                            )}
              ...
              {activeTab === "experience" && (
                <div className="grid gap-6">
                  {userDetails.experience?.map((exp, i) => (
                    <div key={i} className="p-5 md:p-6 bg-white/5 backdrop-blur-md rounded-3xl border border-white/10 hover:border-pink-500/30 transition-all duration-300">
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="text-pink-400 text-sm">{exp.company}</div>
                          </div>
                        </div>
                        <span className="text-xs px-3 py-1 rounded-full bg-purple-500/10 text-purple-300 border border-purple-500/20 whitespace-nowrap">
              ...
              {activeTab === "certificates" && (
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {userDetails.certificates?.map((cert, i) => (
                    <div
                      key={i}
                      className="relative bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl overflow-hidden hover:border-pink-500/30 transition-all duration-300 group flex flex-col"
                    >
                      {cert.cfimage && (
