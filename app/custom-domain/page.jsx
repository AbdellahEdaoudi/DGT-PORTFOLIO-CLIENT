...
export default function CustomDomainPage() {
    ...
    const handleSave = async () => {
        if (!customDomain) return toast.error(t('enterDomain'));
        setIsLoading(true);
        try {
    ...
    const handleVerify = async () => {
        ...
        } catch (e) { toast.error(e.response?.data?.message || t('verificationFailed')); }
        finally { setIsLoading(false); }
    };
    ...
    const confirmRemove = async () => {
        ...
        try {
            ...
            setIsSaved(false);
            setIsVerified(false);
            toast.success(t('domainRemovedSuccess'));
        } catch (e) { toast.error(t('errorRemovingDomain')); }
        finally { setIsLoading(false); }
    };
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
            ...
            <div className="max-w-3xl mx-auto pt-4 px-4 sm:px-6">
                <div className="mb-8 text-center">
                    <h1 className="text-3xl sm:text-4xl font-bold mb-3 bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent">{t('customDomainTitle')}</h1>
                    <p className="text-slate-400 text-base sm:text-lg">{t('customDomainSubtitle')}</p>
                </div>

                <div className="bg-slate-800/50 backdrop-blur-xl rounded-2xl p-5 sm:p-8 border border-slate-700/50 shadow-xl">
                    <div className="mb-8">
                        <label className="block text-sm font-semibold text-slate-300 mb-2 ml-1">{t('yourDomainName')}</label>
                    ...
                    {isSaved && (
                        <div className="animate-in fade-in slide-in-from-top-4 duration-500">
                            <div className={`p-4 sm:p-5 rounded-xl flex flex-col sm:flex-row items-start sm:items-center gap-4 border ${isVerified ? 'bg-green-500/10 border-green-500/20 text-green-400' : 'bg-amber-500/10 border-amber-500/20 text-amber-400'} mb-6`}>
                                ...
                                <div>
                                    <h3 className="font-bold text-lg">{isVerified ? t('domainActiveVerified') : t('verificationPending')}</h3>
                                    <p className="text-sm opacity-80 leading-relaxed">{isVerified ? t('domainConfigured') : t('verifyOwnership')}</p>
                                </div>
                            </div>
                            ...
                            {isVerified ? (
                                <div className="flex flex-col sm:flex-row gap-3">
                                    ...
                                    >
                                        <ExternalLink className="w-4 h-4" /> {t('visitSite')}
                                    </button>
                                    <button
                                    ...
                                    >
                                        {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Trash2 className="w-4 h-4" /> {t('removeDomain')}</>}
                                    </button>
                                </div>
                            ) : (
                                <div className="bg-slate-900/80 p-5 sm:p-6 rounded-xl border border-slate-700/50">
                                        ...
                                        <ol className="list-decimal list-inside space-y-3 sm:space-y-2 ml-1 sm:ml-2">
                                            ...
                                            <li>{t('step3AddRecord')}
                                                <ul className="list-disc list-inside ml-4 sm:ml-6 mt-2 space-y-1 text-slate-400 bg-slate-950/50 p-3 rounded-lg border border-slate-800">
                                                    <li><strong>{t('type')}:</strong> A</li>
                                                    <li><strong>{t('name')}:</strong> @ </li>
                                                    <li><strong>{t('value')}:</strong> 76.76.21.21</li>
                                                </ul>
                                            </li>
                                        </ol>
                                    </div>
                                    ...
                                    <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4 mb-6">
                                        <p className="text-blue-400 text-sm">
                                            <strong>{t('note')}:</strong> {t('dnsPropagationNote')}
                                        </p>
                                    </div>
                    ...
                    <div className="bg-slate-800 rounded-2xl p-6 max-w-md w-full border border-slate-700 shadow-2xl animate-in zoom-in-95 duration-200">
                        <div className="flex items-start gap-4 mb-6">
                            ...
                            <div className="flex-1">
                                <h3 className="text-xl font-bold text-white mb-2">{t('removeCustomDomainTitle')}</h3>
                                <p className="text-slate-400 text-sm leading-relaxed">
                                    {t('removeCustomDomainDesc1')} <span className="text-cyan-400 font-semibold">{customDomain}</span>{t('removeCustomDomainDesc2')}
                                </p>
                            </div>
                            <button
